...
class DogHelper {
    static func allDogs() -> [Dog] {
        let colors: [UIColor] = [UIColor.green, UIColor.black, UIColor.purple, UIColor.orange, UIColor.red]
        ...
        var dogs = [Dog]()
        for i in 0...4 {
            let dog = Dog(profile_picture_url: dogProfileImages[i], name: dogNames[i], favorite_toy: favoriteToys[i], age: dogAge[i], dog_description: dogDescriptions[i], color: colors[i])
            dogs.append(dog)
        }
        return dogs
    }
